...
«Обними меня» находится на клавиатуре под полем ввода. «Волшебный шар» — кнопка-ссылка под сообщением после `/start` или `/help` и под ответами шара: она открывает этот же чат с черновиком `/ask `, который пользователь дополняет и отправляет сам. При смене имени бота обновите ссылку в `interactive.mjs`. На старых клиентах Telegram поддержка заполнения черновика может отличаться; команду всегда можно ввести вручную.
