...
Утренние сообщения начинаются с «Доброе утро!», вечерние — с «Добрый вечер!».
Две кнопки: «🤗 Обними меня» (`/hug`, ответ начинается с «Обнимаю») и «🔮 Волшебный шар» (`/ask`).
Обе кнопки находятся на постоянной клавиатуре под полем ввода. Клавиатура также прикрепляется к ежедневным пожеланиям: вводить `/help` для её появления не обязательно. Нажатие «Волшебный шар» включает ожидание вопроса — следующее сообщение можно написать без команды. Дополнительная кнопка-ссылка под сообщением после `/start` или `/help` и под ответами шара открывает этот же чат с черновиком `/ask `. При смене имени бота обновите ссылку в `interactive.mjs`. На старых клиентах поддержка заполнения черновика может отличаться; основной сценарий с клавиатурой от неё не зависит.
Вопрос можно добавить после `/ask` или отправить следующим текстовым сообщением в течение 15 минут.
`/cancel` отменяет ожидание; другие команды также его сбрасывают. Шар — игра со случайными ответами, не предсказание.
Таблица `conversations` хранит ID чата, срок ожидания и номер обновления, но не вопросы.
Перед обновлением существующего Worker повторно примените `schema.sql`: существующие таблицы и подписки сохраняются.

Новая концепция: утром комплимент + пожелание, вечером только пожелание. Каталог `slot-messages.json` содержит по 750 текстов для каждого времени. Начало календаря — 2026-09-15, задаётся в `content.mjs`. При исчерпании отправка завершается ошибкой вместо повторения старых текстов. Пополняйте базу до окончания периода. Не меняйте дату начала при обычной повторной публикации. При переносе на новый аккаунт выберите дату запуска явно.

Комплименты выбираются из очищенного тёплого списка: эпитеты повторяются через 80 утр, вступительные формулировки меняются. Полные утренние сообщения не повторяются в пределах базы. Это не 750 разных эпитетов.

На прочие личные сообщения, включая медиа и неизвестные команды, случайно выбирается одна из 35 фраз с эмодзи `AUTO_REPLIES` из `content.mjs`. Для голосовых используется отдельный пул из 5 фраз `VOICE_REPLIES`. Выбор независимый на каждом сообщении, повторы возможны. Команды, объятия и ожидание вопроса шару имеют приоритет. Групповые сообщения игнорируются. При сетевом сбое после принятия ответа Telegram повтор webhook теоретически может привести к дублю автоответа.
