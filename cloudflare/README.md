...
На прочие личные сообщения, включая медиа и неизвестные команды, отправляется `AUTO_REPLY` из `content.mjs`. `/start`, `/stop`, `/status`, `/help` имеют приоритет. Групповые сообщения игнорируются. При сетевом сбое после принятия ответа Telegram повтор webhook теоретически может привести к дублю автоответа.
